      ...
      >
        ...
        <nav className="flex-1 px-4 py-6 space-y-2">
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                <Icon
                  className={`w-5 h-5 mr-3 ${isActive ? "text-blue-600" : "text-gray-400 group-hover:text-blue-500"}`}
                />
                <div className="flex-1">
                  <div className={`font-medium ${isActive ? "text-blue-700" : ""}`}>{item.label}</div>
        ...
        <div className="px-4 py-4 border-t border-gray-200 bg-gray-50">
          <div className="flex items-center mb-3">
            ...
            <div className="ml-3">
              <div className="text-sm font-medium text-gray-900">{currentUser?.displayName || "Pavel Fišer"}</div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="bg-white shadow-sm border-b border-gray-200 lg:hidden">
          <div className="flex items-center justify-between h-16 px-4">
            ...
            <h1 className="text-lg font-semibold text-gray-900">{currentMenuItem?.label || "CMS"}</h1>
        ...
        <header className="hidden lg:block bg-white shadow-sm border-b border-gray-200">
          <div className="px-8 py-4">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-2xl font-bold text-gray-900">{currentMenuItem?.label || "Dashboard"}</h1>
                <p className="text-gray-600 mt-1">{currentMenuItem?.description || "Vítejte v administraci"}</p>
              </div>
              <div className="flex items-center space-x-4">
                ...
                <div className="flex items-center space-x-2">
                  <span className="text-sm text-gray-600">{currentUser?.displayName || "Pavel Fišer"}</span>
                  <div className="w-8 h-8 bg-blue-600 rounded-full flex items-center justify-center text-white text-sm font-medium">
                    {currentUser?.displayName ? currentUser.displayName.charAt(0).toUpperCase() : "P"}
                  </div>
                </div>
